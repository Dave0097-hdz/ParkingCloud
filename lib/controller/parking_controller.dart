...
import 'package:parkcloud/controller/model/car_model.dart';
...
class ParkingController extends GetxController {
  ...
  void onInit() {
    super.onInit();
    DatabaseReference obstacleRef = FirebaseDatabase.instance.reference().child('obstacle');
    obstacleRef.onValue.listen((event) {
      var data = event.snapshot.value;
      if (data != null && data is Map<String, dynamic>) {
        updateSlotFromSensor('1', data['lugar1']);
        updateSlotFromSensor('2', data['lugar2']);
        updateSlotFromSensor('3', data['lugar3']);
      }
    });
  }

  void updateSlotFromSensor(String slotId, int sensorValue) {
    switch (slotId) {
      case '1': 
      slot1.value = CarModel(
        booked: sensorValue == 1,
        isParked: sensorValue == 1,
        parkingHours: sensorValue == 1 ? "1" : "",
        name: "",
        paymentDone: sensorValue == 1,
      );
      break;
      case '2': 
  ...
  void calculateparkingAmount() {
    double parkingHours = parkingTimeInMin.value / 60;
    parkingAmount.value = (parkingHours *15).toInt();
  }
  ...
  void updateSlotState(String slotId, int newState) {
  DatabaseReference obstacleRef = FirebaseDatabase.instance.reference().child('obstacle');
  switch (slotId) {
    case '1':
      obstacleRef.update({'lugar1': newState});
      break;
    case '2':
      obstacleRef.update({'lugar2': newState});
      break;
    case '3':
      obstacleRef.update({'lugar3': newState});
      break;
    default:
      break;
  }
